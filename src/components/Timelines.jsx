...
export function Timelines({ timelines, derivedMetrics }) {
  ...
  return (
    <section className="bg-panel border border-border-dim rounded-xl p-5">
      ...
      <div className="mb-6 rounded-lg bg-card border border-white/5 p-4">
        ...
        <p className="text-sm text-slate-400 leading-relaxed">
          ...
          Sequencing uses <strong className="text-slate-300">peak frame numbers</strong>—the video frame at which each segment’s rotation velocity is greatest. 
          The <strong className="text-slate-300">archetype</strong> summarizes the order of those peaks (e.g. trunk → lead arm → pelvis). 
        ...
        {sequencing && (
          <div className="bg-card border border-white/5 rounded-lg p-4">
            ...
            {sequencing.coach_point && (
              <div className="mb-3">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Finding</span>
                <p className="text-base font-medium text-slate-200 mt-0.5">{sequencing.coach_point}</p>
              </div>
            )}
            {sequencing.archetype && (
              <div className="mb-4 inline-flex items-center rounded-lg border-2 border-indigo-400/50 bg-indigo-500/20 px-4 py-2">
                <span className="text-xs font-bold uppercase tracking-wider text-indigo-400/90">Archetype</span>
                ...
                {sequencing.peak_order.map((key, i) => {
                  ...
                  return (
                    <span key={key} className="flex items-center gap-1">
                      <span className="rounded-md bg-slate-700/80 px-2.5 py-1.5 text-sm font-medium text-slate-200">
                        {label} <span className="font-mono text-indigo-300">({frame ?? '—'})</span>
                      </span>
                      {!isLast && (
                        <span className="text-indigo-400 px-0.5" aria-hidden="true">
                          →
                        </span>
                      )}
                    </span>
            ...
            {sequencing.mechanistic_link && (
              <div className="mb-2">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500">What it means</span>
                <p className="text-sm text-slate-300 leading-relaxed mt-0.5">{sequencing.mechanistic_link}</p>
              </div>
            )}
            {sequencing.data_backing && (
              <div className="mb-2">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Data backing</span>
                <p className="text-sm text-slate-500 leading-relaxed mt-0.5">{sequencing.data_backing}</p>
              </div>
            )}
            {sequencing.citation && (
              <p className="text-sm text-slate-500 italic border-l-2 border-slate-600 pl-3 mt-2">
                {sequencing.citation}
              </p>
            )}
        ...
        {mean_bat_speed_P3 && (
          <div className="bg-card border border-white/5 rounded-lg p-4">
            ...
            <p className="text-xl font-mono text-slate-200">
              {mean_bat_speed_P3.value?.toFixed(1)} <span className="text-sm text-slate-500">{mean_bat_speed_P3.unit?.replace(/_/g, '/')}</span>
            </p>
        ...
        {data_considered && (data_considered.derived_metrics?.length > 0 || data_considered.side_metrics?.length > 0) && (
          <div className="bg-card border border-white/5 rounded-lg p-4">
            ...
            {derivedList.length > 0 && (
              <div className="mb-4">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Derived (from pose — derived_metrics.json)</span>
                <ul className="mt-1.5 flex flex-wrap gap-2 mb-3">
                  {derivedList.map((key) => (
                    <li key={key} className="text-sm font-mono text-indigo-300/90 bg-indigo-500/10 rounded px-2 py-1">
                    ...
                    <table className="w-full min-w-[420px] text-sm">
                      <thead>
                        <tr className="border-b border-white/10 bg-white/5">
                          <th className="text-left text-xs font-bold uppercase tracking-wider text-slate-500 px-3 py-2">Metric</th>
{PHASES.map((phase) => (
                              <th key={phase} className="text-center text-xs font-bold uppercase tracking-wider text-slate-500 px-3 py-2">
                      ...
                      <tbody>
                        {metricsWithPhaseData.map((key) => (
                          <tr key={key} className="border-b border-white/5 last:border-0">
                            <td className="px-3 py-2 font-mono text-indigo-200/90 text-left">{formatMetricLabel(key)}</td>
                            {PHASES.map((phase) => (
                              <td key={phase} className="px-3 py-2 text-slate-400 text-center font-mono text-xs">
            ...
            {Array.isArray(data_considered.side_metrics) && data_considered.side_metrics.length > 0 && (
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Side metrics (from side_metrics.json)</span>
                <ul className="mt-1.5 flex flex-wrap gap-2">
                  {data_considered.side_metrics.map((key) => (
                    <li key={key} className="text-sm font-mono text-amber-300/90 bg-amber-500/10 rounded px-2 py-1">
                      {formatMetricLabel(key)}
                    </li>
                  ))}
                </ul>
          ...
          return (
            <div className="bg-card border border-white/5 rounded-lg p-4">
              <h3 className="text-sm font-bold uppercase tracking-wider text-amber-400 mb-4 flex items-center gap-2">
                <span className="w-2 h-2 rounded-sm bg-amber-400 shrink-0" aria-hidden />
                Legend
              </h3>
