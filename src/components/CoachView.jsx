export function CoachView({ coachView }) {
  ...
  return (
    <section className="bg-panel border border-border-dim rounded-xl p-5">
      ...
      {one_line_summary && (
        <div className="rounded-xl bg-indigo-500/10 border border-indigo-500/25 p-4 mb-6">
        ...
        <div className="mb-6">
          ...
          <div className="space-y-3">
            {Object.entries(phase_by_phase_story).map(([phase, text]) => (
              <div key={phase} className="rounded-lg bg-card border border-white/5 p-4">
                <span className="text-xs font-bold text-indigo-400 uppercase">{phase}</span>
                <p className="text-base text-slate-300 mt-1 leading-relaxed">{text}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {Object.entries(what_to_tell_the_player).map(([key, text]) => (
              <div key={key} className="flex gap-2">
                <span className="text-xs font-bold text-emerald-500/80 uppercase shrink-0 w-16">
                  {key}
                </span>
        ...
        <div>
          ...
          <div className="space-y-2">
            {Object.entries(watch_for_on_video).map(([phase, items]) => (
              <div key={phase}>
                <span className="text-xs font-bold text-amber-500/80 uppercase">{phase}</span>
                <ul className="mt-1 space-y-1">
                  {(Array.isArray(items) ? items : [items]).map((item, i) => (
                    <li key={i} className="text-base text-slate-300 leading-relaxed flex gap-2">
