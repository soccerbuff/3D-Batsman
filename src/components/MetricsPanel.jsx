...
export function MetricsPanel({ metrics, error }) {
  if (error) {
    return (
      <div className="p-8 text-center">
        ...
        <div className="text-xs text-slate-600 mt-2">Ensure side_metrics.json is in public/</div>
      </div>
    );
  }
  if (!metrics) return <div className="p-8 text-center text-slate-500 text-base">Loading metrics…</div>;
  ...
  return (
    <div className="flex flex-col gap-6 h-full overflow-y-auto pr-2 pb-4">
      <Section title="Stance & Balance" color="text-teal-400" border="border-teal-500/30">
      ...
      <Section title="Weight Transfer & Movement" color="text-orange-400" border="border-orange-500/30">
      ...
      <Section title="Swing & Impact" color="text-indigo-400" border="border-indigo-500/30">
...
function Section({ title, color, border, children }) {
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className={`grid grid-cols-2 gap-3 p-3 rounded-xl bg-card border ${border}`}>
        {children}
      </div>
    </div>
  );
